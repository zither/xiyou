...
if($zsspd==1){
    ...
    if($m>=1){
        ...
        foreach(array_keys($cwnc) as $key){
            $keycwnc[]=$cwnc[$key];
        }
        ...
        for($x=0;$x<=9;$x++){
            $i=$i+1;
            $cw_index = $ym*10 - 9 + $i;
            $clname= empty($keycwnc[$cw_index]) ? $keycwmz[$cw_index] : $keycwnc[$cw_index];
            if ($clname && !preg_match('/【.+】/', $clname)) {
                $clname = sprintf('【%s】', $clname);
            }
