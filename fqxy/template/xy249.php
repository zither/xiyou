...
if($zsspd==1){
    ...
    if ($cwbz==2) {
        ...
        //获取返回战斗页面
        if ($yymid == 389 || $yymid == 522) {
            include(__DIR__ . '/../ini/pksw_ini.php');
            $npcc = $iniFile->getItem('玩家id', '初始');
            echo "<font color=red>对不起！！你不能对玩家使用宠物捕捉！！！</font>"."<br>";
            $template = sprintf('%s/../template/xy%d.php', __DIR__, $yymid);
            include $template;
            include(__DIR__ . "/../pz/pz01.php");
        } elseif($yymid==130){
            $npcc=($iniFile->getItem('怪物编号','1号'));
